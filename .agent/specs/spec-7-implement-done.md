...
1. **Merge de `origin/main` neste branch.** Descoberta: as Specs 2–6 e 8 (Contexto/Hero, Diagnóstico, Dores, Arquitetura, Iniciativas, Provocações) já estavam implementadas e mergeadas em `main` pelo time, via PRs — este branch havia sido criado antes disso e só tinha Spec 1 + Portal. `git merge origin/main` foi executado sem conflitos. Isso trouxe também uma mudança de navegação feita pelo time: o `Sidebar` antigo foi substituído por uma navegação "Ecossistema" (`GalaxyMap`/`Planet`/`planets.ts`), com `App.tsx` controlando estados `splash` → `galaxy` → `module`.
...
3. **Modo de protótipo em tela cheia:** ao entrar na seção `portal` (clique no planeta, MiniMap, ou navegação sequencial), `App.tsx` agora renderiza um layout dedicado fora do `Header`/`MiniMap` padrão: uma barra superior de 24px (`bg-gi-navy`) com botão "Voltar" (volta para a Ecossistema) à esquerda e "Próximos passos" (navega direto para a seção Provocações) à direita, e abaixo o `PortalShell` em modo `fullscreen` (nova prop), ocupando 100% da viewport restante. `PortalShell` ganhou a logo real da GI Group (`logo-gi-group.png`) no topo da sidebar do protótipo, antes só havia o texto "GI Worker".
